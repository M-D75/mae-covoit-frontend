...
<template>
    ...
    <v-main class="main">
        ...
        <!-- Car model -->
        <BottomMenu ref="BottomMenuRefModelVehicul" :class-name="['model-vehicul']" mode="select-model-vehicul"
            v-on:close="overlay = false" />

        <!-- Select Preference -->
        <BottomMenu ref="BottomMenuRefPreference" :class-name="['preference-choice']" :about="about"
            mode="preference-choice" v-on:close="overlay = false" />
        ...
        async updateCar() {
            if (this.modeDriver) {
                ...
                if (result.status == 0 && this.cars.length > 0) {
                    this.groupeParameters[1].parameters[0].text = this.cars[0].license_plate;
